...
root = pathlib.Path("D:/data/coco-pose-relabel").resolve(strict=True)
...
def plot(
    ...
    if pred.size > 0:
        for pred_one in pred:
            cls, box, kpts = np.split(pred_one, [1, 5])
            cls, box, kpts = cls.astype(np.int32).item(), box.astype(np.int32), kpts.round().astype(np.int32)
            ...
            for (point_x, point_y, visible) in kpts.reshape((-1, 3)):
                if visible == 1:
                    cv2.circle(plotted_img, (point_x, point_y), 2, (0, 255, 255), cv2.FILLED)
                elif visible == 2:
                    cv2.circle(plotted_img, (point_x, point_y), 2, (0, 0, 255), cv2.FILLED)
...
def main():
    ...
    for idx, (image_path, label_path) in tqdm.tqdm(enumerate(zip(image_paths, label_paths, strict=True)),
        ...
        if len(raw_label) > 0:
            ...
            label[:, 1:5] = ultralytics.utils.ops.xywhn2xyxy(label[:, 1:5], image.shape[1], image.shape[0])
            label[:, 5::3] *= image.shape[1]
            label[:, 6::3] *= image.shape[0]
